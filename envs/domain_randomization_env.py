...
class DomainRandomizationWrapperV2(Wrapper, AttrPassthroughMixin):
    def __init__(self,
                 env,
                 noise_std,
                 relative_noise=True,
    ...
    def _get_noisemakers(self, increase_uncertainty):
        noisemakers = []
        ts = []
        for module in self.env.modules.iterlist():
            try:
                time_series = module.time_series
                ts.append(time_series)
            except AttributeError:
    ...
    def randomize_timeseries(self):
        for module, noisemaker, og_ts in zip(self.env.modules.iterlist(), self._noisemakers, self._og_time_series):
            if noisemaker is None:
                assert not hasattr(module, 'time_series')
                continue
    ...
    def reset_timeseries(self):
        for module, og_ts in zip(self.env.modules.iterlist(), self._og_time_series):
            if og_ts is not None:
                module.time_series = og_ts
